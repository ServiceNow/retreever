...
class MRLLoss(torch.nn.Module):
    def __init__(
        ...
        max_tmp_qc: float = None,
        max_tmp_cq: float = None,
        use_mean_finally: bool = True,
        freeze_tmp: bool = True,
        base_loss_type: str = "contrastive",
    ):
        """Multi-Resolution Loss that applies contrastive loss at multiple embedding dimensions.
        
        Creates contrastive losses for dimensions: powers of 2 < encoder_dim, plus encoder_dim.
        For encoder_dim=1024: [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]
        For encoder_dim=768: [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 768]
        
        Args:
            encoder_dim (int): The full encoder dimension
            init_tmp (float): Initial temperature value (frozen for all losses)
            local_loss (bool): Whether to use local loss computation
            sim_measure (str): Similarity measure to use
            use_siglip_loss (bool): Whether to use SigLIP loss
            max_tmp (float): Maximum temperature value
            use_separate_temps (bool): Whether to use separate temperatures for q->c and c->q
            max_tmp_qc (float): Maximum temperature for q->c direction
            max_tmp_cq (float): Maximum temperature for c->q direction
        """
        super(MRLLoss, self).__init__()
        
        self.encoder_dim = encoder_dim
        
        # Compute dimensions: powers of 2 < encoder_dim, plus encoder_dim
        self.dims = []
        power = 1
        while power < encoder_dim:
            self.dims.append(power)
            power *= 2
        self.dims.append(encoder_dim)
        
        self.use_mean_finally = use_mean_finally
        self.base_loss_type = base_loss_type
        
        # Create a ContrastiveLoss for each dimension
        self.losses = torch.nn.ModuleDict()
        for dim in self.dims:
            if base_loss_type == "contrastive":
                self.losses[str(dim)] = ContrastiveLoss(
                    init_tmp=init_tmp,
                    local_loss=local_loss,
                    freeze_tmp=freeze_tmp,  # Always freeze temperature
                    sim_measure=sim_measure,
                    optimize_prior_levels_too=False,
                    optimize_token_loss=False, 
                    optimize_whole_emb=True, # Use full sliced embedding
                    encoder_dim=dim,  # Set to current dim so it uses all of it
                    use_siglip_loss=use_siglip_loss,
                    max_tmp=max_tmp,
                    use_separate_temps=use_separate_temps,
                    max_tmp_qc=max_tmp_qc,
                    max_tmp_cq=max_tmp_cq,
                    use_depth_wise_temps=False,
                )
            elif base_loss_type == "multi_label_contrastive":
                self.losses[str(dim)] = MultiLabelContrastiveLoss(
                    init_tmp=init_tmp,
                    local_loss=local_loss,
                    freeze_tmp=freeze_tmp,
                    sim_measure=sim_measure,
                    max_tmp=max_tmp,
                    optimize_whole_emb=True  # Use full sliced embedding
                )
    ...
    def forward(
        ...
        Args:
            q_embs (torch.Tensor): Question embeddings of shape [batch_size, encoder_dim]
            c_embs (torch.Tensor): Context embeddings of shape [batch_size, encoder_dim]
        
        Returns:
            torch.Tensor: Mean of all contrastive losses across dimensions
        """
        if self.base_loss_type == "contrastive":
            losses = [self.losses[str(dim)](q_embs[:, :dim], c_embs[:, :dim], torch.tensor(0)) for dim in self.dims]
        elif self.base_loss_type == "multi_label_contrastive":
            losses = [self.losses[str(dim)](q_embs[:, :dim], c_embs[:, :dim], torch.tensor(0), labels) for dim in self.dims]
        
        # for dim in self.dims:
        #     # Slice embeddings to the current dimension
        #     q_slice = q_embs[:, :dim]
        #     c_slice = c_embs[:, :dim]
            
        #     # Compute loss for this dimension
        #     # depth is needed but not used
        #     depth = torch.tensor(0)
        #     loss = self.losses[str(dim)](q_slice, c_slice, depth)
        #     losses.append(loss)
        
        # Return sum or mean of all losses
        if self.use_mean_finally:
...
class MatryoshkaLoss(torch.nn.Module):
    ...
    """
    Code for Matryoshka Representation Learning, adapted from https://github.com/RAIVNLab/MRL/blob/main/MRL.py#L11 .
    """

    def __init__(self, nesting_list: List[int], relative_importance: List[float] = None, **kwargs):
        """Objective function for combining contrastive losses at different representation levels.

        Args:
            nesting_list (List[int]): list of dimensions (in ascending order), each dimensions determining the first n to use as representation. A contrastive loss will be computed for each n.
            relative_importance (List[float], optional): weights for each contrastive loss. Defaults to list of 1s.
        """
        super(MatryoshkaLoss, self).__init__()

        self.criterion = ContrastiveLoss(**kwargs)
    ...
    def __call__(
        ...
        """Computes Matryoshka loss.

        Args:
            q_embs (torch.Tensor): question representations
            c_embs (torch.Tensor): positive context representations
        Returns:
            torch.Tensor: total contrastive loss, combining all losses per representation dimension.
        """

        # Calculate losses for each dimension and stack them. This is still O(N)
        losses = torch.stack(
            [
                self.criterion(q_embs[:, start_dim:end_dim], c_embs[:, start_dim:end_dim], depth)
                for start_dim, end_dim in self.nesting_list
            ]
        )
        ...
        rel_importance = (
            ...
            else torch.tensor(self.relative_importance).to(losses.device)
        )
